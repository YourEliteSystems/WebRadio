...
This API is backed by the core `NavigationManager` and exposed to plugins through the plugin API.
...
* **NavigationManager** – manages sections and items.
* **PluginRuntime** – registers plugin navigation during plugin start and removes it on stop.
* **Sidebar UI** – renders the navigation tree generated by the core.
